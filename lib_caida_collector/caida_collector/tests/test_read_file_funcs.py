...
class TestReadFileFuncs:
    def test_get_url(self, mock_caida_collector, tmp_caida_collector):
        """Tests that the URL collected from Caida is accurate

        Get an example html and ensure that the URL is what we expect
        """

        test_url = ("http://data.caida.org/datasets/as-relationships/"
    ...
    def test_write_cache_file_mock(self, mock_caida_collector, decoded_path):
    ...
    def test_write_cache_file(self, tmp_caida_collector):
    ...
    def test_read_file(self, 
        ...
        with decoded_path.open(mode="r") as f:
            decoded = [x.strip() for x in f.readlines()]
